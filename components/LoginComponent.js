...
class RegisterTab extends Component {
    ...
    getImageFromCamera = async () => {
        ...
        let cameraRollPermission = await ImagePicker.getMediaLibraryPermissionsAsync();
        if (cameraPermission.status === 'granted' && cameraRollPermission.status === 'granted') {
            ...
            cameraRollPermission = await ImagePicker.requestMediaLibraryPermissionsAsync();
            if (cameraPermission.status === 'granted' && cameraRollPermission.status === 'granted') {
                let capturedImage = await ImagePicker.launchCameraAsync({
                    allowsEditing: true,
                    aspect: [4, 3],
    ...
    getImageFromLibrary = async () => {
        let librrayPermission = await ImagePicker.getMediaLibraryPermissionsAsync();
        if (librrayPermission.status === 'granted') {
            ...
            librrayPermission= await ImagePicker.requestMediaLibraryPermissionsAsync();
            if (librrayPermission.status === 'granted') {
                let libraryImage = await ImagePicker.launchImageLibraryAsync({
                    mediaTypes: ImagePicker.MediaTypeOptions.All,
                    allowsEditing: true,
    ...
    processImage = async (imageUri) => {
        let processedImage =await ImageManipulator.manipulateAsync(
            imageUri,
            [{ rotate: 90}, { flip: ImageManipulator.FlipType.Vertical },{resize:{height:3000} }],
        ...
        return(
            ...
            <View style={styles.container}>
                <View style={styles.imageContainer}>
                    <Image 
                        source={this.state.imageUrl ? {uri: this.state.imageUrl} : require('./images/logo.png')} 
...
class Login extends Component{
    render(){
        return (
        <NavigationContainer initialRouteName="Login"  independent>
